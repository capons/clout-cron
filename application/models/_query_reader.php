...
class _query_reader extends CI_Model
{
	...
	public function __construct()
    {
		...
		# write DB connection
        $this->dbWrite = $this->load->database(GROUPNAME_CRON_WRITE,TRUE);
		# read DB connection
		$this->dbRead = $this->load->database(GROUPNAME_CRON_READ,TRUE);
		# memcached for read queries
		if(extension_loaded('memcached')){
			log_message('debug', '_query_reader/__construct:: [1] memcached for read queries');
			$this->memcached = new Memcached();
			$this->memcached->addServer(HOSTNAME_CRON_READ, 11211);
		}
	...
	{	
		...
		foreach($keys AS $key) $dataForQuery['_'.strtoupper($key).'_'] = str_replace('_ESCAPE_','\\',$queryData[$key]);
	...
	public function load_queries_into_cache()
	{
		...
		log_message('debug', '_query_reader/load_queries_into_cache:: [1] queries='.json_encode($queries));
	...
	function get_count($queryCode, $queryData = array())
	{
		...
		# due to replication delays, read the immediate result from write db for this server's transactions
		if(!(!empty($queryData['__source']) && $queryData['__source'] == 'read')) $this->dbRead = $this->dbWrite; 
	...
	function get_mongo_query_by_code($queryCode, $queryData = array())
	{
		...
		log_message('debug', '_query_reader/get_mongo_query_by_code:: [2] query='.json_encode($query));
