...
class _mongo extends CI_Model
{
	...
	function run($query, $strict=FALSE)
	{
		...
		log_message('debug', '_mongo/run:: [1] query='.json_encode($query).' strict='.$strict);
		...
		log_message('debug', '_mongo/run:: [3] result='.json_encode($result));
	...
	function get_count($query)
	{
		...
		log_message('debug', '_mongo/get_count:: [1] query='.json_encode($query));
		
		$mQuery = $this->mysql_to_mongo($query);
		log_message('debug', '_mongo/get_count:: [2] mQuery='.json_encode($mQuery));
		
		try {
			$response =  $this->db->selectCollection($mQuery['table'])->count($mQuery['array']['conditions']);
		} catch (Exception $e){
			$response = array();
		}
		return $response;
	...
	function get_row_as_array($query, $dontConvert=FALSE)
	{
		log_message('debug', '_mongo/get_row_as_array');
		log_message('debug', '_mongo/get_row_as_array:: [1] query='.json_encode($query));
		
		$mQuery = $dontConvert? $query: $this->mysql_to_mongo($query);
		log_message('debug', '_mongo/get_row_as_array:: [2] mQuery='.json_encode($mQuery));
		
		try {
			$response = $this->db->selectCollection($mQuery['table'])->findOne($mQuery['array']['conditions'], $mQuery['array']['values']);
		} catch (Exception $e){
			$response = array();
		}
		return $response;
	...
	function get_list($query, $dontConvert=FALSE)
	{
		log_message('debug', '_mongo/get_list');
		log_message('debug', '_mongo/get_list:: [1] query='.json_encode($query));
		...
		log_message('debug', '_mongo/get_list:: [3] response='.json_encode($response));
			...
			foreach($parts2 AS $part2){
				...
				if(strpos($part2, 'mongo_distance(') !== FALSE){
					...
					if(!empty($latLonParts) && count($latLonParts) > 2) {
						$array2 = array_merge($array2, array($latLonParts[2] => array('$near'=>array(
							'$geometry'=> array(
									'type'=>'Point',
				...
				# LIKE operand
					...
					if(substr($like,0,1) == '%' && substr($like,-1) == '%') $array2 = array_merge($array2,  array(trim($parts[0])=>
						array('$regex'=>trim($like,'%'), '$options'=>'i') ));
						
					else if(substr($like,0,1) == '%') $array2 = array_merge($array2, array(trim($parts[0])=> array('$regex'=>trim($like,'%').'^', '$options'=>'i')) );
					
					else if(substr($like,-1) == '%') $array2 = array_merge($array2, array(trim($parts[0])=> array('$regex'=>'^'.trim($like,'%'), '$options'=>'i')) );
					else $array2 = array_merge($array2, array(trim($parts[0]) => cast_number(trim($like)) ));
				}
				...
				# IN operand
					...
					if(count($items) > 0) $array2 = array_merge($array2, array(trim($parts[0])=> array('$nin'=>$items)) );
				}
					...
					if(count($items) > 0) $array2 = array_merge($array2, array(trim($parts[0])=> array('$in'=>$items)) );
				}
				
				
				# NULL operands
				else if(strpos($part2, ' is not null') !== FALSE) {
					$array2 = array_merge($array2, array(trim(str_replace('is not null','',$part2)) => array('$exists'=>TRUE) ));
				}
				else if(strpos($part2, ' is null') !== FALSE) {
					$array2 = array_merge($array2, array(trim(str_replace('is null','',$part2)) => array('$exists'=>FALSE) ));
				}
				
				# any other character operands
				else if(strpos($part2, '<=') !== FALSE) {
					$parts = explode('<=',$part2);
					if(count($parts) > 1) $array2 = array_merge($array2, array(trim($parts[0])=>array('$lte'=>cast_number(trim(trim($parts[1]),"'")) )) );
				}
				else if(strpos($part2, '>=') !== FALSE) {
					$parts = explode('>=',$part2);
					if(count($parts) > 1) $array2 = array_merge($array2, array(trim($parts[0])=>array('$gte'=>cast_number(trim(trim($parts[1]),"'")) )) );
				}
				else if(strpos($part2, '!=') !== FALSE || strpos($part2, '<>') !== FALSE) {
					$parts = strpos($part2, '!=') !== FALSE? explode('!=',$part2): explode('<>',$part2);
					if(count($parts) > 1) $array2 = array_merge($array2, array(trim($parts[0])=>array('$ne'=>cast_number(trim(trim($parts[1]),"'")) )) );
				}
				else if(strpos($part2, '<') !== FALSE) {
					$parts = explode('<',$part2);
					if(count($parts) > 1) $array2 = array_merge($array2, array(trim($parts[0])=>array('$lt'=>cast_number(trim(trim($parts[1]),"'")) )) );
				}
				else if(strpos($part2, '>') !== FALSE) {
					$parts = explode('>',$part2);
					if(count($parts) > 1) $array2 = array_merge($array2, array(trim($parts[0])=>array('$gt'=>cast_number(trim(trim($parts[1]),"'")) )) );
				}
				else if(strpos($part2, '=') !== FALSE) {
					$parts = explode('=',$part2);
					if(count($parts) > 1) $array2 = array_merge($array2, array(trim($parts[0])=>cast_number(trim(trim($parts[1]),"'")) ) );
				}
				
			}
			
			if(count($array2) > 0) array_push($array1, $array2);
		}
		# Add the OR condition
		if(count($parts1) > 1) $array['$or'] = $array1;
		else if(count($parts1) == 1) $array = current($array1);
