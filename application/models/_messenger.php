...
class _messenger extends CI_Model {
	...
	function email($message, $sendMethod = 'server')
	{
		...
		if($sendMethod == 'ses'){
			...
			if(!empty($messageDetails['fileurl'])) $m->addAttachmentFromFile(basename($messageDetails['fileurl']), $messageDetails['fileurl'], mime_content_type($messageDetails['fileurl']));
