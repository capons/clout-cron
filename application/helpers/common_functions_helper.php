...
function server_curl($url, $data=array()) 
{
...
function run_on_api($url, $data, $runType='POST', $returnType='array')
{
		...
		else if($runType == 'XML')
		{
			if(!empty($data['file_url'])){
				$file = fopen($data['file_url'], 'r');
				curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: text/xml'));
				curl_setopt($ch, CURLOPT_URL, $url);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($ch, CURLOPT_POSTFIELDS,  fread($file, filesize($data['file_url'])) );
				curl_setopt($ch, CURLOPT_POST, 1);
...
function strstr_($string, $pattern, $beforePattern=FALSE, $position=1) 
{
	...
	if(strlen($pattern) > 0 && strpos($string, $pattern) !== FALSE){
		...
		if(count($parts) > 0){
			# get up to the nth position
			if($position > 1){
				if(!$beforePattern) $final = implode($pattern, array_slice($parts, ($position - 1)) );
				else $final = implode($pattern, array_slice($parts, 0, $position));
			}
			else {
				if(!$beforePattern) {
...
# bundle one column by another in multi-row array
function bundle_by_column_multi_array($array, $bundleKeyColumn, $bundleValueColumn, $keepRow=FALSE)
{
	$final = array();
	$firstItem = current($array);

	if(!empty($firstItem[$bundleKeyColumn]) 
		&& ($bundleValueColumn == '' || ($bundleValueColumn != '' && !empty($firstItem[$bundleValueColumn])))
	){
		foreach($array AS $row) {
			if($keepRow) $final[$row[$bundleKeyColumn]] = $row;
			else if($bundleValueColumn != '') {
				if(empty($final[$row[$bundleKeyColumn]])) $final[$row[$bundleKeyColumn]] = array();
				if(!in_array($row[$bundleValueColumn], $final[$row[$bundleKeyColumn]])) array_push($final[$row[$bundleKeyColumn]], $row[$bundleValueColumn]); 
			}
		}
	}
...
# get checkby date based on the oldest date given for that range
function get_checkby_date($oldestDate, $periodBack){
	if(!empty($oldestDate)){
