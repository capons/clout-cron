...
import com.clout.cron.batch.jobitems.StoresItem;
...
public class BusinessDataItemReader implements ItemReader<BusinessItem> { 
	...
	private final static int GOOGLE_API_QUOTA_PER_THREAD = 1500;
		...
		try {
			...
			while(task.hasNext()) {
				...
				try {
					...
					store.setCategory(info.getNaics().getCodeDetails());
					//store.setCategoryLevel2Id(categoryLevel2ToNaicsMapping.get(info.getNaics().getCode()));
